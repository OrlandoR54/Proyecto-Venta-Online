...
<?php
               ...
               AND mh_products_prod_id=mh_products_prod_id";
                         ...
                         if($result->num_rows >0){
                             while($row=$result->fetch_assoc()){
                                echo "Suma",$total=$row["SUM(carri_subt)"] ;
          ...
          $sql3="INSERT INTO `mh_fact_cabec_vent`(`fc_vent_id`, `fc_fecha_vent`, `fc_vent_iva`, `fc_vent_total`, `mh_persons_per_id`, `fc_vent_estado`, `fc_estado`) 
           ...
           $sql4="SELECT `fd_vent_id` FROM `mh_detal_vent`, `mh_fact_cabec_vent` WHERE $codigo=mh_persons_per_id";
             $result = $conn->query($sql4);
             if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    $idcabecera=$row["fd_vent_id"];
                }
            }  else{
              ...
              /* AND mh_products_prod_id=mh_products_prod_id ";*/
               ...
               if ($result->num_rows > 0) {
                   while ($row = $result->fetch_assoc()) {
                       echo $IDdelProductoA=$row["mh_products_prod_id"] ;
                        echo $SubtotalCadaProductoA=$row["carri_subt"] ;
                        echo $cant=$row["carrit_prod_vent"];
             ...
             $sql7="UPDATE `mh_fact_cabec_vent`  
               ...
               WHERE $idcabecera=fc_vent_id";
                ...
                while ($row = $result3->fetch_assoc()) {
                 echo "ID CARRITO DE COMPRA" ,$idcarrito=$row["carrit_id"];
                }
                }else{
                     echo "No recupero id de la cabecera";
                 }   
